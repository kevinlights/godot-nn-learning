...
@onready var message = $GeneticInfo/Message
...
var initializeWithThatWeights = [0.80117901334392, -0.66901837430714, -0.52845502307405, 0.05880103066812, -0.73453974858919, -0.0796239889588, -0.12428542470532, 0.30518798441505, 0.85899762900494, -0.08190592403411, 0.47327935482996, -0.68734815583593, -0.24638892347618, 0.95990391231876, -0.47872723589436, -0.8503956144581, -0.59480842259092, -0.45214571929147, -0.34495462784201, -0.60743953447193, 0.97109704294534, -0.63724922311874, 0.45990465709022, -0.368995994007, -0.59722126449732, -0.79992202477882, 0.96715749459791, -0.73086854761705, -0.00594714042551, 0.75774561997752, -0.45376233604866, -0.96554940595748, -0.17393083869739, 0.41143160782741, -0.95331035386667, 0.9103566358767, 0.47438286446786, -0.14477792127091]
...
func displayPopulation():
	...
	for i in range(genetic.population.size()):
		...
		# "input_nodes": 3,		"hidden_nodes": 6,		"output_nodes": 2,
		# 3*6 + 6*2 + 6 + 2 = 18+12+8 = 38
...
func onPlayerHit(k):
	...
	for i in range(instances.size()):
		...
		if instance.k == k:
			...
			instances.erase(instance) # 从数组中移除个体，注意这里的 instances 的索引会发生变更，就不能再使用 k 来遍历了
			balls.erase(ball)
			break
...
func onBotHit(k):
	...
	if k < len(instances):
		message.text = instances[k].bot_name + " hit!"
	else:
		message.text = "k=" + str(k)
...
func restart():
	...
	avg_lifetime.text = "Avg Rating: " + str(round(average))
